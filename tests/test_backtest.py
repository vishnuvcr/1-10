...
def synthetic_data(n=80):
    ...
    return pd.DataFrame({"Open":close.values,"High":close.values+1,"Low":close.values-1,"Close":close.values,"Volume":1000}, index=idx)

def test_indicator_columns_exist():
    out = add_indicators(synthetic_data())
    assert {"SMA21","RSI14","PrevRSI14","LongSignal","ShortSignal"} <= set(out.columns)
...
def test_empty_trades_do_not_crash_backtest():
    trades,equity,metrics = run_backtest(synthetic_data(),costs=CostModel(brokerage_per_order=0,slippage_bps=0))
    assert trades.empty
